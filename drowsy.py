...
def get_gaze_ratio(eye_points,landmark,frame,gray):
            ...
            for s in obj:
                ...
                if ear<eyethresh:                                                       #if EAR is less than threshold value
                    ...
                    if eye_closure_duration>0.4 and eye_closure_duration<2.2:
                        print("The Driver is Drowsy")
                        cv2.putText(frame, "The Driver is Drowsy", (5, 30), cv2.FONT_HERSHEY_SIMPLEX,
                                    0.7, (0, 0, 255), 2)
                    count=0
                    alarm_status=False

                if lip_of_distance>yawn_thresh and horizontal_mouth_dist<=41.2:         #if the driver is yawning
                    cv2.putText(frame,"The Driver is Yawning",(7,30),cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,0,255),2)
                    yawn_check=1
                else:
                    alarm_status=False
                    if yawn_check==1:
                ...
                gaze_ratio_left_eye = get_gaze_ratio([36, 37, 38, 39, 40, 41], shape_m,frame,gray)         #call to get gaze ratio function
                gaze_ratio_right_eye = get_gaze_ratio([42, 43, 44, 45, 46, 47], shape_m,frame,gray)
